...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class CustomRotateTowards : Action
    {
        ...
        public SharedTransformList RotateTowardsPoints;
        ...
        public override void OnStart()
        {
            ...
            IntwaypointCounter = IntwaypointCounter % RotateTowardsPoints.Value.Count;
            if(IntwaypointCounter < 0)
            {
                IntwaypointCounter += RotateTowardsPoints.Value.Count;
            }
        }
        ...
        {
            if (RotateTowardsPoints.Value[IntwaypointCounter] == null || RotateTowardsPoints.Value[IntwaypointCounter] == null)
            {
                return Quaternion.Euler(targetRotation.Value);
            }
            var position = RotateTowardsPoints.Value[IntwaypointCounter].transform.position - transform.position;
            if (onlyY.Value)
            {
                position.y = 0;
            }
        ...
        public override void OnReset()
        {
            ...
            onlyY = false;
            RotateTowardsPoints.Value[IntwaypointCounter] = null;
